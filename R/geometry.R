...
new_geo <- function(data, fname, tin = NULL) {
  if (is.null(fname)) fname <- "<unspecified>"
  stopifnot(is.character(fname) && length(fname) == 1)
  stopifnot(inherits(data, "list"))

  if (is.null(tin))
    tin <- tin.matrix(cbind(data$x, data$y), ikle = data$ikle, ipobo = data$ipobo)

  structure(
    list(header = list(title = data$title,
...
validate_geo <- function(x) {
  ...
  stopifnot(all(c("header", "tin", "elevation") %in% names(x)))
  ...
  if (any(c(nrow(x$tin$points), length(x$elevation)) != x$header$npoin))
    stop("Number of points in 'tin' and 'elevation' must be equal to 'npoin' in the header!", call. = F)
...
#' @name geo
#' @export
geo.data.frame <- function(x, fname = NULL, col_x = "x", col_y = "y", col_elev = "z", col_elem = "id", title = "", ...) {
...
#' @param dem If \code{x} is of type \code{t2d_tin}: a DEM from which the values will
#' be interpolated to the TIN mesh points. This input will be forwarded to
#' \code{\link{interpol}} (argument \code{src}; see descriptions therein for supported
#' types). You can pass further arguments for interpolation, e.g. the numer of
#' neighbours \code{n} etc.
#' @name geo
#' @export
geo.t2d_tin <- function(x, fname = NULL, ..., dem, title = "") {
  ...
  data <- list(title = title,
               ...
               npoin = nrow(x$points),
               ndp = 3L,
               elevation = interpol(x, dem, output = "numeric", ...))

  validate_geo(new_geo(data, fname, x))
}
...
#' and element \code{elevation}, the mesh point elevations.
#' @export
read_geo <- function(fname) {
  ...
  if (x_head$nbv1 != 1)
    stop("Expected geometry file with only one variable, this one contains ", x_head$nbv1, " variables.", call. = F)

  # read variable (mesh point elevation) values
  x_var <- read_slf_variable(fname, x_head$seek_head, 1, 1, x_head$precision, x_head$npoin)

  c(x_head, list(elevation = c(x_var$values)))
}
...
write_geo.t2d_geo <- function(x) {
  ...
  x_head <- c(x$header, list(x = x$tin$points[,1], y = x$tin$points[,2],
                             ...
                             nbv1 = 1L, varnames = "BOTTOM", varunits = "M", date = Sys.time()))
  ...
  x_var <- list(
    ...
    time = 0,
    values = array(x$elevation, dim = c(x_head$npoin, 1, 1))
  )
