...
In addition we need some more R packages to run this demonstration, which include [tidyverse](https://www.tidyverse.org), [raster](https://cran.r-project.org/web/packages/raster/index.html), and [sf](https://r-spatial.github.io/sf/index.html). The packages can be installed directly from CRAN.
...
The `telemac` package provides the function `tin()` to initialise a TIN the model mesh will be based on. Internally it uses the R package [RTriangle](https://cran.r-project.org/web/packages/RTriangle/index.html) to perform triangulation with the Triangle algorithm. In the example the catchment boundary is taken and triangulation performed starting with a resolution of 90 m (`s = 90`) of TIN points along the catchment boundary. Furthermore, the resulting triangles shall not be larger than 10,000 m^2 (`a = 100^2`) and the angles within the triangles not smaller than 30 degrees (`q = 30`).
...
For this step the `telemac` package provides function `geo()`. The function accepts several input types, e.g. the name of an existing SELAFIN file from which the geometry is read. However, in this demonstration we want to generate a new geometry file based on our triangulated DEM. The interpolation will be conducted using an approach of inverse distance weighting (IDW) of nearest neighbours as implemented in function `idw()` of package [gstat](https://cran.r-project.org/web/packages/gstat/index.html).
...
The steering file is a simple text file that contains a number of key--value pairs (delimited by characters `:` or `=`), the steering parameters for a TELEMAC simulation. The `telemac` package comes with function `cas()` to initialise a `t2d_cas` object with steering parameters associated with a steering file. The function provides differen options, e.g. providing the name of an existing file or creating a template configuration if no argument is given. The template configuration contains a number of arbitrary parameters, in this case defining a strong rainfall event of 66 mm over 4 hours, dry initial conditions, a total simulation period of one day with adaptive temporal resolution using a finite-volume scheme, and output printout every 3600 timesteps. Note that there is a huge number of possible steering parameters that are not included in the template file (for these the respective default settings are used by TELEMAC).
...
```{r cas_object_mod}
str(cas_obj)
cas_obj[["VARIABLES FOR GRAPHIC PRINTOUTS"]] <- "H,M"
```
...
```{r t2d_obj}
t2d_obj <- t2d(title = "Test setup", wdir = "t2d_test", exec = "telemac2d.py",
...
The model can be run either directly or via R interface `simulate_t2d()`. However, TELEMAC-2D runs, depending on domain size, spatial and temporal resolution, and numerical settings, can take quite a while. Using the R interface to conduct simulations makes only sense if you are sure the model run will not take too long.
...
Note that the results of a previous simulation run can be imported as well.

```{r import_results}
t2d_obj$res <- results(system.file("telemac/results.slf", package = "telemac"),
                       log = system.file("telemac/test_run.log", package = "telemac"),
                       vars = "water depth")
```

The simulation results are represented as a `t2d_res` object with header information, the underlying mesh as `t2d_tin` object, a log of TELEMAC's runtime messages, and the actual data as a tidy `data.frame`. For a first investigation it is always good to have a look at the log file. The runtme log is stored in the `t2d_res` object and is accessible via `t2d_obj$res$log` (not shown here). Otherwise there is a print method to show some general information.
...
There is also a function `write_results()` to write data into a SELAFIN file. This can be used, for instance, to reduce the amount of data or write restrospectively corrected data. Note that in this example we have only one out of originally two variables imported that will now be written into a SELAFIN file.
